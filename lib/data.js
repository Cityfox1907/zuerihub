...
export const TRADE_ICONS = { 'Restaurant': '🍽️', 'Bar': '🍺', 'Café': '☕', 'Hotel': '🏨', 'Bäckerei': '🥐' }
...
export const KEYWORD_ICONS = {
  ...
  'Kunstmuseum': '🎨', 'Wissenschaftsmuseum': '🔬',
  'Naturhistorisches Museum': '🦕', 'Historisches Museum': '🏰',
  'Kulturmuseum': '🎭', 'Kindermuseum': '🧒',
  'Allgemeines Museum': '🏛️',
  'Kinder': '👶',
  'Schwimmbad': '🏊', 'Strandbad': '🏖️', 'Badesee': '🌊',
  'Freibad': '☀️', 'See': '💧', 'Wellness': '♨️',
}

export const SHOP_ICONS = {
  'Mode & Fashion': '👗', 'Sportbekleidung': '🏃',
  'Second-Hand & Vintage': '♻️', 'Einkaufszentren': '🏬',
}
...
export function getEmoji(spot) {
  ...
  if (spot.source === 'badi') return KEYWORD_ICONS[spot.keyword] || '🏊'
...
export function getSpotTag(spot) {
  ...
  if (spot.source === 'badi') return spot.keyword || 'Schwimmbad'
...
export function getSourceInfo(source) {
  const map = {
    gastro: { label: 'Essen & Trinken', cls: 'src-gastro' },
    attr: { label: 'Kultur & Natur', cls: 'src-attr' },
    museum: { label: 'Museum', cls: 'src-museum' },
    shop: { label: 'Shop', cls: 'src-shop' },
    fun: { label: 'Unterhaltung', cls: 'src-fun' },
    badi: { label: 'Badis & Seen', cls: 'src-badi' },
  }
...
/** Normalize a badeort entry (different JSON structure) */
function normalizeBadi(p) {
  const kw = p.found_with_keyword || ''
  const ptd = p.primary_type_display || ''
  // Map to simplified categories
  let keyword = 'Schwimmbad'
  if (kw === 'Strandbad' || ptd === 'Strand') keyword = 'Strandbad'
  else if (kw === 'Badesee' || ptd === 'See') keyword = 'Badesee'
  else if (kw === 'Freibad') keyword = 'Freibad'
  else if (ptd === 'Wellness-Center' || ptd === 'Sauna') keyword = 'Wellness'
  return {
    id: p.place_id || '', name: p.name || '', addr: p.address || '',
    lat: p.latitude || 0, lng: p.longitude || 0,
    r: p.rating || 0, rv: p.user_rating_count || 0,
    gmaps: p.google_maps_url || '', web: p.website || '',
    trade: '', cuisines: [],
    keyword, subcat: '', source: 'badi',
  }
}
...
export async function loadAllData(onProgress) {
  ...
  cb(5)
  const badiRes = await fetch('/data/badeorte_kanton_zuerich.json').then(r => r.ok ? r.json() : [])
  cb(6)

  const gastro = (gastroRes.orte || gastroRes || []).map(normalizeGastro)
  const attr = (attrRes.orte || []).map(p => normalizePlace(p, 'attr'))
  const KINDER_KEYWORDS = /kinder|kids|spielparadies|indoorspielplatz|monkey.*town|trampolin|spielzänti|minicity|piratolino|playland|springding|paradisimo|montelino/i
  const fun = (funRes.orte || []).map(p => {
    const spot = normalizePlace(p, 'fun')
    if (spot.keyword === 'Indoorspielplatz' || KINDER_KEYWORDS.test(spot.name)) {
      spot.keyword = 'Kinder'
    }
    return spot
  })
  ...
  const badis = (Array.isArray(badiRes) ? badiRes : badiRes.orte || []).map(normalizeBadi)

  const all = [...gastro, ...attr, ...museen, ...fun, ...shops, ...badis]

  return { gastro, attr, fun, museen, shops, badis, all }
}
...
export function getFavKey(source) {
  const map = { gastro: 'zh-favs', attr: 'zh-attr-favs', fun: 'zh-fun-favs', shop: 'zh-shop-favs', museum: 'zh-museum-favs', badi: 'zh-badi-favs' }
  return map[source] || 'zh-favs'
}

/** Get all favorite IDs from localStorage */
export function getAllFavIds() {
  const keys = ['zh-favs', 'zh-attr-favs', 'zh-fun-favs', 'zh-shop-favs', 'zh-museum-favs', 'zh-badi-favs']
  const ids = []
