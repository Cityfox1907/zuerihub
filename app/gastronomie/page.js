...
export default function GastronomiePage() {
  ...
  const filtered = data.gastro
    .filter(p => p.r > 0 && (!search || p.name.toLowerCase().includes(search.toLowerCase()) || p.addr?.toLowerCase().includes(search.toLowerCase())))
    .sort((a, b) => (b.r * b.rv) - (a.r * a.rv))
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 1480, margin: '0 auto', padding: '1.5rem' }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Suche nach Name oder Adresse…" style={{ width: '100%', maxWidth: 400, padding: '.6rem 1rem', borderRadius: 10, border: '1.5px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontSize: '.85rem', marginBottom: '1.5rem', outline: 'none' }} />
